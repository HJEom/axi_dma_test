...
with tf.Session(config=tf.ConfigProto(log_device_placement=False)) as sess:
    ...
    for itr in range(iteration+1):
        ...
        train_, cost_ = sess.run([train, cost],
                feed_dict={in_img_f : train_low_img_uint8, label_img_f : train_high_img_uint8})

        ##### forward pass to calculate psnr
        layer3_uint8 = sess.run(layer3_out,
                feed_dict={in_img_f : train_low_img_uint8})

        ##### calculate psnr
        psnr_, summary_ = sess.run([psnr, merged_summary],
                feed_dict={label_img_uint8 : train_high_img_uint8, out_img_uint8 : layer3_uint8, in_img_f : train_low_img_uint8, label_img_f : train_high_img_uint8})
        ...
        if itr%100 == 0:
            ...
            ##### forward pass to calculate psnr
            layer3_uint8 = sess.run(layer3_out,
                    feed_dict={in_img_f : test_low_img_uint8})
